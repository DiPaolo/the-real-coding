...
class FileProcessingWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        row_len_pix = rect.width() - indent * 2
        total_strip_len = rows * row_len_pix
        bytes_per_pix = self._total_bytes / total_strip_len

        print(f'{rows}x{cols}   {total_strip_len}, {self._total_bytes} bytes === {bytes_per_pix:.1f} bpp, pix_per_line={row_len_pix}')
        ...
        file_num = 0
        cur_line_num = 0
        next_start_pt = rect.topLeft() + QPoint(indent, indent)
        for filename, file_size in self._file_infos.items():
            ...
            cur_line_len_pix = rect.width() - next_start_pt.x() - indent
            while pix_left >= cur_line_len_pix:
                qp.drawRect(next_start_pt.x(), next_start_pt.y(), cur_line_len_pix, internal_size)
                next_start_pt.setX(indent)
                next_start_pt.setY(next_start_pt.y() + cell_size)
                pix_left -= cur_line_len_pix
                cur_line_len_pix = rect.width() - next_start_pt.x() - indent
                cur_line_num += 1
                print(f'next line ({cur_line_num})')

            qp.drawRect(next_start_pt.x(), next_start_pt.y(), pix_left, internal_size)
            next_start_pt.setX(next_start_pt.x() + pix_left + indent * 2)
